...
| --- | --- | --- | --- |
| 0 | Ground truth and tooling | half a day | Honest data claims, a test runner, a branch |
| 1 | Cut the marketing surface | 3 days | Two locales, nine routes, one name, a landing that sells |
...
- [ ] **0.1 Branch.** `git checkout -b feat/product main`. All product phases merge here, `feat/product` merges to `main` per phase.
...
- [ ] **0.3 Test runner.** `npm i -D vitest @vitest/coverage-v8`. Add `"test": "vitest run"` and `"test:watch": "vitest"`. `vitest.config.ts` with `@` alias matching `tsconfig.json`. First test: `tests/unit/currency.test.ts` covering `currencyForCountry("IN") === "INR"`, `"CA"`, `"DE"`, `undefined`.
- [ ] **0.4 Playwright.** `npm i -D @playwright/test` and `npx playwright install chromium`. `playwright.config.ts` pointing at `http://localhost:3000`, one smoke test that loads `/` at 390 by 844 and asserts no horizontal overflow (`document.documentElement.scrollWidth <= 390`).
- [ ] **0.5 Upgrade the Vercel CLI** globally: `npm i -g vercel@latest`. Re-run `vercel link` to confirm the project.
- [ ] **0.6 Commit** each of the above separately. Lint 0, tsc clean, `npm test` green.
